...
import { CLS_RESOURCE, ROOT_URL } from "~/util/constants.js";
import { saveRow, deleteRow, updateRow } from "~/util/api";
import { polyglot } from "jet-locales/ru.js";

export default class ResourceFormView extends JetView {
  config() {
    this.item = {};
    return {
      rows: [
              ...
              cols: [
                {
                  ...
                  id: "save",
                  click: () => saveRow(CLS_RESOURCE, this.item)
                },
                {
                  view: "button",
                  value: polyglot.t("form.delete"),
                  id: "delete",
                  click: () => deleteRow(CLS_RESOURCE, this.id)
                },
                {
                  view: "button",
                  value: polyglot.t("form.update"),
                  id: "update",
                  click: () => updateRow(CLS_RESOURCE, this.item, this.id)
                }
              ]
            }
          ]
  ...
  urlChange(view, url) {
    ...
    webix
      .ajax()
      .get(ROOT_URL + CLS_RESOURCE + "/" + this.id)
      .then(data => {
