...
export default class DataView extends JetView {
  ...
  urlChange(view, url) {
    ...
    webix
      .ajax()
      .get(ROOT_URL + CLS_CROP_VARIETY + "/" + url[0].params.id)
      .then(data => {
        $$(FORM_NAME).setValue(data.json().name);
        $$(FORM_NUMBER).setValue(data.json().number);
        $$("combo1").setValue(data.json().clsCropByIdCrop);
      });
  }

  saveRow() {
    const urlPost = ROOT_URL + CLS_CROP_VARIETY + ACTION_CREATE;
    const urlGet = ROOT_URL + CLS_CROP + "/" + $$("combo1").getValue();
    ...
    webix
      ...
      .get(urlGet)
      .then(data => {
        item.clsCropByIdCrop = data.json();
  ...
  updateRow(id) {
    const urlPut = ROOT_URL + CLS_CROP_VARIETY + ACTION_UPDATE;
    const urlGet = ROOT_URL + CLS_CROP_VARIETY + "/" + id;
    const urlCombo = ROOT_URL + CLS_CROP + "/" + $$("combo1").getValue();
    let item;
  ...
  deleteRow(id) {
    const url = ROOT_URL + CLS_CROP_VARIETY + "/" + id;
