...
export default class DataView extends JetView {
  ...
  urlChange(view, url) {
    ...
    webix
      .ajax()
      .get(ROOT_URL + CLS_DISTRICT + "/" + url[0].params.id)
      .then(data => {
        $$(FORM_NAME).setValue(data.json().name);
        $$(FORM_NUMBER).setValue(data.json().number);
        $$("combo1").setValue(data.json().clsRegionByIdRegion);
      });
  }

  saveRow() {
    const urlPost = ROOT_URL + CLS_DISTRICT + ACTION_CREATE;
    const urlGet = ROOT_URL + CLS_REGION + "/" + $$("combo1").getValue();
  ...
  updateRow(id) {
    const urlPut = ROOT_URL + CLS_DISTRICT + ACTION_UPDATE;
    const urlGet = ROOT_URL + CLS_DISTRICT + "/" + id;
    const url = ROOT_URL + CLS_REGION + "/" + $$("combo1").getValue();
    let item;
  ...
  deleteRow(id) {
    const url = ROOT_URL + CLS_DISTRICT + "/" + id;
    webix
      .ajax()
      .del(url)
      .then(data => this.setBlank());
