...
export default class DataView extends JetView {
  ...
  init() {
    webix
      .ajax()
      .get(ROOT_URL + CLS_EQUIPMENT_TYPE)
      .then(function(data) {
        const list = $$("combo1")
          .getPopup()
          .getList();
  ...
  saveRow() {
    const urlPost = ROOT_URL + CLS_EQUIPMENT_KIND + ACTION_CREATE;
    const urlGet =
    ...
    webix
      ...
      .get(urlGet)
      .then(data => {
        item.clsEquipmentTypeByIdEquipmentType = data.json();
  ...
  updateRow(id) {
    const urlPut = ROOT_URL + CLS_EQUIPMENT_KIND + ACTION_UPDATE;
    const urlGet = ROOT_URL + CLS_EQUIPMENT_KIND + "/" + id;
    const urlCombo =
      ROOT_URL + CLS_EQUIPMENT_TYPE + "/" + $$("combo1").getValue();
    let item;
  ...
  deleteRow(id) {
    const url = ROOT_URL + CLS_EQUIPMENT_KIND + "/" + id;
    webix
      .ajax()
      .del(url)
      .then(data => this.setBlank());
